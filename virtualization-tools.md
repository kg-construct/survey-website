...
Morph-RDB extension [17] enables Ontology-Based Data Access to streaming data sourcesby extending R2O [17] mapping rules with streaming support (S2O mapping rules) (T5) and querying these data sources with SPARQLStream (T13a).
Morph-RDB’s SPARQLStream is inspired by C-SPARQL [7] and SNEEql [15] but with support for streaming windows and SPARQL 1.1 aggregates [96].
...
#### Ontop v4.1.1 (2013)
Ontop [99] virtualizes access to relational SQL databases and OpenGIS by translating SPARQL queries into SQL queries.
It leverages R2RML [32] mapping rules or Ontop Mapping Language (T5) to perform this translation, and extensions for accessing OpenGIS data (T1, T2).
...
In previous generations of Ontop, ontological entailment was supported with a query rewriting approach using the PerfectRef algorithm [21], but that was abandoned as mapping saturation proved more efficient.
...
#### Optique (2013)
Optique [66, 48] leverages R2RML mapping rules [32] (T5) to access data such as relational databases, triple stores, temporal databases, data streams, etc.
Optique uses STARQL [89] as query language for streaming data which is translated into SQL queries (T1, T2).
These SQL queries are executed on Optique’s DSMS ExaStream [109, 89].
...
Morph-RDB [94] – also discussed in section 5.3 as materialization implementation – supports virtualization for relational databases such as MySQLPostgresSQL, H2, and MonetDB or files in CSV format (T1, T2) through R2RML [32] mapping rules (T5).
...
Morph-xR2RML [86], also discussed in Section 5.3 as ETL implementation, is a fork or Morph-RDB [94] which translates SPARQL queries into SQL queries and MongoDB queries using xR2RML mapping rules (T1, T2, T5, T13a). Morph-xR2RML does not
...
SparqlMap-M [110] is an extension of SparqlMap [111] to provide virtualization over CSV files, relational databases, and non-relational databases using R2RML mapping rules [32] (T5).
SparqlMap-M extends R2RML [32] to support data transformations and conditions (T6) which are executed during the schema transformation (T7).
SparqlMap-M analyses SPARQL queries [96] to reduce the R2RML mappings to the minimum for answering the query and translates to SQL to execute the query on the the database engine (T13a).
...
Morph-streams++ [78] is the successor of the extended Morph-RDB and leverages SPARQLStream [18] queries.
Morph-streams++ uses existing Distributed Stream Management Systems (DSMS) to execute SPARQLStream queries with R2RML mapping rules [32] (T5).
...
Supported data sources and formats in Morphstreams++ depend on the underlying DSM: currently the [Esper](http://www.espertech.com/esper) and Global Sensors Network (GSN) [1] (T1, T2) are supported by Morph-streams++.
...
#### Squerall v0.2 (2019)
Squerall [82] answers SPARQL queries [96] over heterogeneous data sources that are mapped to RDF with RML [42] (T5) and FnO functions [35, 36] (T6).
...
#### Ontario (2019)
Ontario [44] provides virtualization over a set of SPARQL endpoints [96] and non-SPARQL database interfaces using RML mapping rules [42] (T5).
...
#### Obi-Wan (2020)
Obi-Wan [16] is a Java-based (T8) virtualization implementation for data stored in relational and non-relational databases.
